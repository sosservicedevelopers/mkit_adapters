...
namespace AdapterMonListOfTourizm.ServiceContract
{
    public class GetListOfTourizmDetailsWebServices : IGetLisencesSoap/*, IDisposable*/
    {
        ...
        ListOfMonumProt[] IGetLisencesSoap.GetListProtectedMonum(int year)
        {
            try
            {
                ...
                ListOfMonumProt[] lst = new ListOfMonumProt[q.Count()];
                int ind = 0;
                foreach (var item in q.ToList())
                {
                    lst[ind] = new ListOfMonumProt { Name = item.Name, Address = item.Address, Contacts = item.Contacts, CurrentStatus = item.CurrentStatus, DictDistrict = item.DictDistrict, DictFinSource = item.DictFinSource, Director = item.Director, LegalDocs = item.LegalDocs, Ploshad = item.Ploshad };
                    ind++;
                }
